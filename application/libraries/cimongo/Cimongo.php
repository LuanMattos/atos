...
class Cimongo extends Cimongo_extras {
        ...
        /**
         * Num rows de uma coleção
        **/
        public function num_row_collection($collection = ""){

            $client         = new \MongoDB\Client();
            $base           = new Cimongo_base();
            $base_name      = $base->dbname;
            $this->_client  = $client;
            $client         = $client->{$base_name}->{$collection};
            $client->find();
            $size           = $client->count();
            return $size;
        }
         ...
         *  @since v1.0.0
         */
        public function insert($collection = "", $insert = array(),$return = FALSE) {
                ...
                try {
                    $query = $this->db->selectCollection($collection)->insertOne($insert, array("w" => $this->query_safety));
                    if (isset($insert['_id'])) {
        ...
        /**
         * Converte o dado retornado de uma consulta mongo em json
        **/
        public function mongoToJson($data){
            if(!empty($data)){
                $bson = BSON\toJSON(BSON\fromPHP($data));
                return $bson;
            }
            return false;
        }
        /**
         * Converte o dado retornado de uma consulta mongo em array
         **/
        public function mongoToPhp($data){
            if(!empty($data)){
